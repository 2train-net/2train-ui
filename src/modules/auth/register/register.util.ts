...
import { REQUIRED_EXCEPTION_TEXT, MIN_NUMERIC_DIGIT_EXCEPTION_TEXT } from 'shared/constants';
...
export const REGISTER_FORM_SCHEMA = Yup.object().shape<ICreateAccountFormValues>({
  ...
  type: Yup.mixed<UserType>().oneOf([UserType.PersonalTrainer, UserType.Customer]).required(REQUIRED_EXCEPTION_TEXT),
  ...
  phone: Yup.string().required(REQUIRED_EXCEPTION_TEXT).min(8, MIN_NUMERIC_DIGIT_EXCEPTION_TEXT),
});
