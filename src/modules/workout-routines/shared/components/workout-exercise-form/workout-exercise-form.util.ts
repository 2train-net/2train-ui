...
export const WORKOUT_EXERCISE_FORM_SCHEMA = Yup.object().shape({
  ...
  sets: Yup.number().typeError(NUMBER_TYPE_EXCEPTION_TEXT).required(REQUIRED_EXCEPTION_TEXT),
  reps: Yup.number()
    .typeError(NUMBER_TYPE_EXCEPTION_TEXT)
    .when('focus', {
      is: WorkoutExerciseFocus.REPS,
      then: Yup.number().typeError(NUMBER_TYPE_EXCEPTION_TEXT).required(REQUIRED_EXCEPTION_TEXT),
